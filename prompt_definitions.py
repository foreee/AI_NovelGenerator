...
# =============== 1. 核心种子设定（雪花第1层）===================
core_seed_prompt = """\
你是一位资深的网络小说创作作家，请用"雪花写作法"第一步构建故事核心：
主题：{topic}
类型：{genre}
...
伏笔操作：埋设(A线索)→强化(B矛盾)...
认知颠覆：★☆☆☆☆
本章简述：[描述本章的主要内容，包含伏笔操作、意外笑点等要素。字数在100-200字之间]
...
- 每章简述需包含核心作用、悬念密度、伏笔操作、意外笑点等要素。字数在100-200字之间。
- 合理安排节奏，确保整体悬念曲线的连贯性。
...
要求：
- 合理安排节奏，确保整体悬念曲线的连贯性，至少补回已有章节中的两个伏笔。
- 故事逻辑清晰，不要偏离架构过渡发散
- 在生成{number_of_chapters}章前不要出现结局章节。
- 不要使用markdown格式。
...
请完成第 {novel_number} 章的正文，字数要求{word_number}字或英文单词，至少设计下方2个或以上具有动态张力的场景：
1. 对话场景：
   - 潜台词冲突（表面谈论A，实际博弈B）
   - 权力关系变化（通过非对称对话长度体现）
   - 至少1处双关语暗示未来危机
...
写作要求（非常重要！必须遵守）：
- 聚焦在设定的主题和类型，不要出现和主题类型无关的内容
- 使用通俗易懂的人类沟通方式表达，但要有逻辑严谨性
- 对话的部分使用短句，不要冗长描述，便于阅读和理解。 
- 不要出现大量重复词汇或相似的语句；
...
你是一位资深的小说创作作家，使用网络小说常见的写作风格，创作下一章节的正文。
主题：
{topic}
...
依据前章结尾剧情，开始完成第 {novel_number} 章的正文，字数要求{word_number}字或英文单词，确保与前章结尾衔接流畅，但不重复！
至少设计下方1个或以上具有动态张力的场景：
1. 对话场景：
   - 潜台词冲突（表面谈论A，实际博弈B）
   - 权力关系变化（通过非对称对话长度体现）
   - 至少1处双关语暗示未来危机
...
# - 全局摘要：
# {global_summary}
# 结尾时回收旧悬念/创造新悬念/抛出新危机/颠覆某个认知/神转折等。
...
# 不用经常出现类似“他知道、这只是一个开始、更大的挑战还在前方等待着他”之类相似的话术。